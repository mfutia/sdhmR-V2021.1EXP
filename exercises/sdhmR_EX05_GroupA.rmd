...
#load presence/absence data 
  ...
  pres.abs <- get(load("~/University of Vermont/Classes/sdhmR/sdhmR-V2021.1/data/module02/rang.PPsA.RData")) 
  # load presence:absence data
  head(pres.abs, 2) # examine -- the coordinates w/ no NAs are the tr.wgs_x 
```
  
```{r}
  load("~/University of Vermont/Classes/sdhmR/sdhmR-V2021.1/data/module02/pied.framesR.RData")
  # 
  rang.bufR # examine cropping raster
  plot(rang.bufR) 
```
...
layers <- c(etpt5, etpt6, etptsprin, exp1nrm, exp3nrm, exp5nrm, mindyrav, pradswdi, precwhal, precwinte, taveshal, 
tavesprin, tmaxshal, tmaxsumme, rough, topos)

# build the raster stack
  pied.topoRANG_stack <- stack(etpt5, etpt6, etptsprin, exp1nrm, exp3nrm, exp5nrm, mindyrav, pradswdi, precwhal, precwinte, taveshal, tavesprin, tmaxshal, tmaxsumme, rough, topos) # create a raster stack
  pied.topoRANG_stack # examine stack
  
# extract from stack 
  t1_RANG <- extract(pied.topoRANG_stack, pres.abs[, c("tr.wgs_x", "tr.wgs_y")]) # extract values from raster stack
  head(t1_RANG, 2) # examine extracted matrix 
  
```
This is causing issues-- there are NAs in predictors bc it's not punching out correctly.  
