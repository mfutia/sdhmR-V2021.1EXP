...
```{r}
  ...
  # plot(pied.bufptR) # examine raster plot --> don't really need
```
...
# loop for extracting topo data w/raster stack
  ...
  for (i in 1:length(img.list)) {
    # r1 <- crop(raster(img.list[i]), pied.bufptR) # crop pred var raster to buffer raster --> can skip this
...
```{r}
  rang.trTOPO <- cbind(pres.abs, t1) %>% # bind to train dataframe
