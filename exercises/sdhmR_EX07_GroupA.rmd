...
# model 1 vs. 2 predictions
  ...
  head(mod1.pred)
```
For both, I got error messages that "glm.fit: fitted probabilities numerically 0 or 1 occurred", but I think that's ok, based on the videos. For question 3, I'll use model 2.
...
# modify spatial map to modelling domain; use pied.bufptR from pied.framesR.RData Mod2
# load some frames
  setwd(path.mod2)
  load("pied.framesR.RData") # load buffers: use pied.bufptR
  pied.range.bufR # examine buffered pts raster

# multiply extent by buffered pts 

  plot(rang.probLR) # examine prob prediction map
  plot(pied.range.bufR, add = T, col = "grey60") # examine plot modelling domain
```

```{r}
  # range.pdom <- pied.range.bufR * rang.probLR # multiply prob raster by domain raster
  # range.cdom <- pied.bufptR * pied.clasRF # multiply clas raster by domain raster
  range.pdom <- crop(rang.probLR, pied.range.bufR)
  range.cdom<- crop(rang.clasLR, pied.range.bufR)
  
  # extent(range.pdom)
  # extent(range.pdom)
  # extent(pied.range.bufR) --> All are the same extent?

# giggle plots
  par(mfrow = c(1, 2))
  plot(range.pdom, legend = T, axes = T, main = "Probability Map") # plot clipped probability map
  plot(sf::st_geometry(states), add = T, lwd = 1.5) # add state boundaries
  plot(range.cdom, legend = F, axes = T, main = "Classification Map") # plot clipped classified map
  plot(sf::st_geometry(states), add = T, lwd = 1.5)  # add state boundaries
  par(mfrow = c(1, 1))
...
# Classification threshold as a stand--alone scalar
mod.cut.EX7 <- mod.cut[[2]]

#plot - also saved this manually
LRmaps_EX07

save(all.acc, mod.cut.EX7, LRmaps_EX07, file = "range.LR_EX07.RData")
```
