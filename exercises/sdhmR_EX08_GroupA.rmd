...
# PartRange_GAM2 <- (RANG106 ~ etpt_5.img+lo(mind_yr_av.img)+lo(prad_sw_di.img)+lo(prec_w_hal.img)+lo(rough_1k.img)+lo(tmax_s_hal.img)+lo(topos.img), family = "binomial", data = tr_RANG) 
summary.Gam(PartRange_GAM2)

# Stepwise GLM
PartRange_GAM2 <- step.Gam(PartRange_GAM,scope=list(
    "etpt_5.img" = ~1 + etpt_5.img + s(etpt_5.img, 3) + s(etpt_5.img, 5),
    "mind_yr_av.img" = ~1 + mind_yr_av.img + s(mind_yr_av.img, 3) + s(mind_yr_av.img, 5),
    "prad_sw_di.img" = ~1 + prad_sw_di.img + s(prad_sw_di.img, 3) + s(prad_sw_di.img, 5),
    "prec_w_hal.img" = ~1 + prec_w_hal.img + s(prec_w_hal.img, 3) + s(prec_w_hal.img, 5),
    "rough_1k.img" = ~1 + rough_1k.img + s(rough_1k.img, 3) + s(rough_1k.img, 5),
    "tmax_s_hal.img" = ~ 1 + tmax_s_hal.img + s(tmax_s_hal.img, 3) + s(tmax_s_hal.img, 5),
    "topos.img" = ~1 + topos.img + s(topos.img, 3) + s(topos.img, 5)), trace = F)
summary(PartRange_GAM2)
```

```{r}
PartRange_GAM2.fit <- 100 * (1 - PartRange_GAM2$deviance/PartRange_GAM2$null.deviance)
PartRange_GAM2.fit # 24.4 (up from full GLM)
PartRange_GAM2$deviance # 5,769.3 (down from full GLM)
...
GAM2_acc.sum = rbind(PartRange_GAM2.acc, PartRange_GAM2.accB)
```
...
plot(rang.clasGAM, legend = T, axes = T, main = "Classification Map")
plot(st_geometry(states), add = T, lwd = 1.5)
```
...
# load(file = "C:/Users/mttft/Desktop/UVM/UVM Courses/SDHM_R/sdhmR-V2021.1EXP/data/RangeData/GAM_Results.Rdata")
```
