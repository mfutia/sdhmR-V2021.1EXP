...
* Construct a set of GAMs from fully linear to a maximum of 5 splines
  * Use lowess smoother rather than default spline --> UPDATE: use default splines!
...
mod1.GAM_RANGE <- gam(RANG106 ~ etpt_5 + s(mind_yr_av, 2) + s(prad_sw_di, 2) + s(prec_w_hal, 2) + s(rough_1k, 2) + s(tmax_s_hal, 2) + s(topos, 2), family = binomial, data = tr_RANG)

summary(mod1.GAM_RANGE)
```

I only used smoothers on the variables listed above, not etpt_5. 

```{r, warning=FALSE}
# GAM model 2: smoothers=3
mod2.GAM_RANGE <- gam(RANG106 ~ etpt_5 + s(mind_yr_av,3) + s(prad_sw_di,3) + s(prec_w_hal,3) + s(rough_1k,3) + s(tmax_s_hal,3) + s(topos,3), family = binomial, data = tr_RANG)

summary(mod2.GAM_RANGE) # model #2 summary
...
mod3.GAM_RANGE <- gam(RANG106 ~ etpt_5 + s(mind_yr_av,4) + s(prad_sw_di,4) + s(prec_w_hal,4) + s(rough_1k,4) + s(tmax_s_hal,4) + s(topos,4), family = binomial, data = tr_RANG)
...
mod4.GAM_RANGE <- gam(RANG106 ~ etpt_5 + s(mind_yr_av,5) + s(prad_sw_di,5) + s(prec_w_hal,5) + s(rough_1k,5) + s(tmax_s_hal,5) + s(topos,5), family = binomial, data = tr_RANG)
...
Model 2 is my top pick-- it's consistently in the top few models based on cross-fold validation. Model 4 would also be a good pick but is less parsimonious? These models are all pretty similar in performance, none are great. 
...
rang.probGAM <- predict(rangeDOM, mod2.GAM_RANGE, filename="rang.probGAM.img", type="response", fun=predict,index=2, overwrite=TRUE)
rang.probGAM
