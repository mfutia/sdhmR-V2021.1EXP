...
```{r}
setwd(path.mod2)
pied.trTOPO <- read_csv("pied_trTOPO.csv")
colnames(pied.trTOPO)
```
I'm not going to add a new column, to preserve data structure, but groupings and column numbers are (based on the HTML doc): 

**topography** = "exp1nrm.img"    "exp3nrm.img"    "exp5nrm.img" "prad_sw_di.img" "rough_1k.img"  "topos.img" = [,c(8:10,12,15,20)]

**temperature** =   "tave_s_hal.img" "tave_sprin.img" "tmax_s_hal.img" "tmax_summe.img" = [,16:19]

**precipitation** = "etpt_5.img", "etpt_6.img", "etpt_sprin.img" "prec_w_hal.img" "prec_winte.img" "mind_yr_av.img" = [,c(5:7,13:14,11)]
...
The panel plot of all variables' correlations is pretty hard to see here, but zooming in it looks like several are correlated, and several have "significant" but unimportant correlations. I'll look at groups of predictors for a better examination of correlations. 


```{r}
# numeric correlations
  cut.point <- 0.7 # set cutpoint for correlation
  c1 <- cor(pied.trTOPO[,c(8:10,12,15,20)], use = "pairwise.complete.obs", 
    method = "spearman") # est. correlation
  c1 # examine
  c2 <- subset(c1 > cut.point | c1 < -cut.point) # matrix of cor>cutpoint
  c2 # examine; FALSE indicates cor<cutpoint 
...
# plot correlations using modified panel.cor function
  pairs(pied.trTOPO[,c(8:10,12,15,20)], lower.panel = panel.smooth,
  upper.panel = panel.cor, main = "Topo Variables")
```
...
# numeric correlations
  cut.point <- 0.7 # set cutpoint for correlation
  c1 <- cor(pied.trTOPO[,c(16:19)], use = "pairwise.complete.obs", 
    method = "spearman") # est. correlation
  c1 # examine
  c2 <- subset(c1 > cut.point | c1 < -cut.point) # matrix of cor>cutpoint
  c2 # examine; FALSE indicates cor<cutpoint 
...
# plot correlations using modified panel.cor function
  pairs(pied.trTOPO[,c(16:19)], lower.panel = panel.smooth,
...
# numeric correlations
  cut.point <- 0.7 # set cutpoint for correlation
  c1 <- cor(pied.trTOPO[,c(5:7,13:14,11)], use = "pairwise.complete.obs", 
    method = "spearman") # est. correlation
  c1 # examine
  c2 <- subset(c1 > cut.point | c1 < -cut.point) # matrix of cor>cutpoint
  c2 # examine; FALSE indicates cor<cutpoint 
...
# plot correlations using modified panel.cor function
  pairs(pied.trTOPO[,c(5:7,13:14,11)], lower.panel = panel.smooth,
...
# estimate VIP values => AIC & Adj deviance
  tr.vip <- as.data.frame(pied.trTOPO[,c(2, 8:10,12,15,20)]) # keep only P/A & predictors - had to make this a df to get GLM to work
...
# built basic barplot if desired
  ...
  x.labs <- names(tr.vip[2:v.stop]) # x-axis labels
   ...
   barplot(dev.fit[, 2], col = "darkgreen", ylim = ylim.r, main = "topo VIPs", 
    ylab = "adj.D2", names = x.labs) # barplot
  abline(h = 0) # add horizontal line
  abline(mean(dev.fit[, 2]), 0, lt = 3) # ref lines; dash=mean adj.dev 
```
The most important topo variable is rough_1k. 

```{r}
# estimate VIP values => AIC & Adj deviance
  tr.vip <- as.data.frame(pied.trTOPO[,c(2, 16:19)]) # keep only P/A & predictors - had to make this a df to get GLM to work
  pres <- 1 # column for presence:absence
  v.start <- 2 # column start predictor variables
  v.stop <- ncol(tr.vip) # last column predictor variables
  v.num <- v.stop - 1 # number predictor variables
...
# built basic barplot if desired
  ...
  abline(mean(dev.fit[, 2]), 0, lt = 3) # ref lines; dash=mean adj.dev 
```
Most important temp variable is tmax_s_hal. Will work to select just this one since they're all correlated. 

```{r}
# estimate VIP values => AIC & Adj deviance
  tr.vip <- as.data.frame(pied.trTOPO[,c(2,5:7,13:14,11)]) # keep only P/A & predictors - had to make this a df to get GLM to work
  pres <- 1 # column for presence:absence
  v.start <- 2 # column start predictor variables
  v.stop <- ncol(tr.vip) # last column predictor variables
  v.num <- v.stop - 1 # number predictor variables
...
# built basic barplot if desired
  ...
  abline(mean(dev.fit[, 2]), 0, lt = 3) # ref lines; dash=mean adj.dev 
```
Most important are eptp_5, etpt_6, adn mind_yr_av. 
...
```{r}
pied.tr_MORT <- pied.trTOPO %>% 
  select(FNETID, SPPRES106, tr.wgs_x, tr.wgs_y, prad_sw_di.img, rough_1k.img, tmax_s_hal.img, tave_s_hal.img, etpt_6.img, prec_w_hal.img, mind_yr_av.img)
```
...
# write out data files if desired
  setwd(path.mod2)
  write.csv(pied.tr_MORT, file = "pied.tr_MORT.csv", row.names = F) # save .csv - training data
  # save(pied.tr_MORT, file = "pied.tr_MORT.RData") # save .RData - pres/abs plus stacked raster
  save(pied.tr_MORT, file = "pied.tr_MORT.RData") # save .RData - stacked raster
  st_write(pied.tr_MORT, dsn = ".", layer = "pied.tr_MORT", driver = "ESRI Shapefile",
