...
c1 <- cor(all_data[, c(18:33)], use = "pairwise.complete.obs", method = "spearman")
c1
...
```
From the precipitation variables, etpt_5 and mind_yr_av have similarly high adjusted deviance. Based on temperature, tmax_s_hal has the highest adjusted deviance. Lastly, based on topography, rough_1k has the highest adjusted deviance.
```
...
```
From the precipiation variables, I elimiated etpt_6, and etpt_sprin because they are highly correlated with mind_yr_av and have lower adjusted deviance. I also eliminated prec_winte because of its strong correlation with prec_w_hal and the lower adjusted variance. Of the temperature variables, I only retained tmax_s_hal because it is highly correlated with the other three temperature variables and has the highest adjusted deviance. Lastly, I eliminated the topographic variables exp1nrm, exp3nrm, and exp5nrm due to their low adjusted deviance.
```
...
st_write(tr_RANG, dsn = ".", layer = "tr_RANG_SF", driver = "ESRI Shapefile", delete_layer = T, delete_dsn = T)
```
