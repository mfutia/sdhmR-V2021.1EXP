...
Model fit seems alright based on sens/spec, but TSS is really low-- about 50%, same as random chance. AUC also not great, especially for random forest. Modify MAXENT CV code:
...
# simple loop for xfold
  for (i in 1:xfold) {
    ...
    mx <- randomForest(mod.form(tr_RANG, 2, 5), importance = T, 
    keep.forest = T, data = tr) # maxent model on training
...
dat5 <- cbind(spp, tr_RANG[2],mod2.predXF) # build dataframe: true data plus the 2 validation s
  head(dat5, 2)
  
  mod.cutCV <- optimal.thresholds(dat5, opt.methods = c("ObsPrev")) # threshold=PREVALENCE
  mod1.accCV <- presence.absence.accuracy(dat5, threshold = mod.cutCV$mod2.predXF)
  tss <- mod1.accCV$sensitivity + mod1.accCV$specificity - 1 # code TSS metric
  dat5.acc_CV <- cbind(mod1.accCV[1:7], tss) # bind all metrics
  head(dat5.acc_CV)

```

```{r cross validation - code from online}
