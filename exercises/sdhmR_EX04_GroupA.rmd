...
* **Range**: the total dataset, including all spatial locations of seedlings, mortality, and persistence (This is what we'll use)
...
#   simple rounding by 0.5 deg, creates ~50 km buffer in Utah
  extb <- extent(min(spp106pr_RANG$wgs_xF) - 0.5, max(spp106pr_RANG$wgs_xF) + 0.5, 
                 min(spp106pr_RANG$wgs_yF) - 0.5, max(spp106pr_RANG$wgs_yF) + 0.5)
...
setwd(path.gis)
t1 <- terra::rast("templateR_wgs.img")
	t1 #CRS is WGS 84

# extract buffered points from template & rasterize
  rang.bufSP <- as_Spatial(rang.bufSF) # convert to sp spatialpolygon class
  rang.bufptR <- crop(raster(rang.bufSP, t1), rang.bufSP) # t1 is template from above
  rang.bufptR # examine

   #how do you plot a raster in ggplot?
```
There may be an error in the raster of the WGS84 reference, but it looks like the CRS is WGS84. How do we know what resolution & projection to use?
...
# extract coords, cell id of coords, & build dataframe
  f1 <- sp::coordinates(rang.bufptR) # get spatial coords from rang.bufptR
  f2 <- cellFromXY(rang.bufptR, f1) # extract cell number from buffered extent
  f3 <- as.data.frame(cbind(f1, f2)) # build datframe of x,y & cell number
  head(f3, 2) # examine
  tail(f3, 1) # f2 value should eq. ncell in rang.bufptR; below
  ncell(rang.bufptR) # should eq. f2 value above
...
# convert dataframe to spatial object
  rang.fnetSF <- st_as_sf(f3, coords = c("x", "y"), 
...
# save objects
  setwd(path.mod2) # output path
  save("rang.fnetSF", "rang.fnetDF", file = "rang.fnet.RData")
```
...
# convert species x,y to spatial coordinate; tru.pres from above
  ...
  p1 <- sp::coordinates(tru.xy) # set spatial coords from rang.bufptR

# extract FNETID of presence from different modelling frames
  pres.fnetid <- cellFromXY(rang.bufptR, p1) # FNETID of presence from pt-buffered poly
...
# create species dataframe with FNETIDs
  tru.presFNET <- cbind(pres.fnetid, tru.xy, spp106pr_RANG[,4]) # add fishnet ID to tru.pres
  names(tru.presFNET)[1] <- "FNETID" # name change 
  head(tru.presFNET, 2) # examine

# giggle plots
  plot(rang.bufptR, col = "gray70", legend = F, main = "Points in modelling frame")  # main plot
...
# extract FNETID of modelling frame; many different frames can be used
  bufpt.fnetid <- raster::extract(rang.bufptR, rang.fnetDF[c("x", "y")])
...
# create modelling dataframe with FNETIDs
  tru.bufptFNET <- cbind(rang.fnetDF, bufpt.fnetid) # bind modelling frame w/FISHNET
  head(tru.bufptFNET, 2) # examine
  
# some internal checking
  length(tru.bufptFNET$bufpt.fnetid) # number FNETIDs in tru.bufptFNET
  ncell(rang.bufptR) # should equal above
  table(tru.bufptFNET$bufpt.fnetid)[[1]] # number of FNETIDs in rang.bufptR
  length(which(is.na(tru.bufptFNET$bufpt.fnetid))) # No. NAs -- This all checks out -AM
```

```{r}
#write rang.bufptR to a .img file
writeRaster(pied.range.bufSF, filename = "pied.range.bufSF", format = "HFA", overwrite = T) 
```
...
# examine dataframes
  ...
  head(tru.bufptFNET, 2) # modelling dataframe
  table(tru.bufptFNET$bufpt.fnetid)[[1]] # number of FNETIDs in rang.bufptR
  head(tru.presFNET, 2) # spp locations dataframe
  dim(tru.presFNET)[1] # number spp locations - these will be removed before making pseudo-abs

# begin merge: NOTE merge by=c("sort vars") & all.y=T options
  m1 <- merge(rang.fnetDF, tru.bufptFNET, by = c("f2", "x", "y"), all.y = T)
  head(m1, 2) # examine: FNETID no longer ranked but not to worry ...
  rang.indexFNET <- merge(m1, tru.presFNET, by.x="f2", by.y = c("FNETID"), all = T) # final merge:assign DF name - why are my cols different? 
  ...
  # internal checking
  length(rang.indexFNET$FNETID) # does it match w/above ?? no
  length(rang.indexFNET$f2) #yes ?... 
  table(rang.indexFNET$in.modFR)[[1]] # does it match w/above ?? yes !
  table(rang.indexFNET$SPPRES106)[[1]] # does it match w/above ?? yes !
...
# merge with true presences for predictor extraction
  ...
  rang.PPsA <- merge(tru.presFNET, psu.srs2, by = c("FNETID", "RANG106", "wgs_xF", "wgs_yF"), 
    all = T) # merge
...
# create common x,y from tru pres and pseudo-abs x,y's
  rang.PPsA[352:355, ] # note diff [X,Y] coords for pres=0 & =1 -- 1'S have wgs_xF, 0's have x 
  #abs coords that represent fishnet coords, not the real coords as in the presences

# create new vars wgs_x & wgs_y; used later in raster stack extraction
  rang.PPsA$tr.wgs_x <- ifelse(rang.PPsA$RANG106 == 0, rang.PPsA$x, rang.PPsA$wgs_xF)
  rang.PPsA$tr.wgs_y <- ifelse(rang.PPsA$RANG106 == 0, rang.PPsA$y, rang.PPsA$wgs_yF) 
...
# resulting data frame is model training data set; outfile if desired
  setwd(path.mod2)
  write.csv(rang.PPsA, file = "rang_PPsA.csv", row.names = F) # save .csv
  
  save(rang.PPsA, file = "rang.PPsA.RData") # save .RData
  
  st_write(rang.bufSP, dsn = ".", layer = "rang.bufSP", driver = "ESRI Shapefile",
    delete_layer = T, delete_dsn = T) # output shapefile
```
