...
It's easier to build a reliable classification model based on environmental variables that vary dramatically between presence and absence sites. Large variation in environmental variables increases the discriminatory power of the model because they can provide a stronger signal:noise ratio. If environmental variables differ by a small amount between presence and absence sites, it's statistically harder to discriminate between weak differences in species' environmental preferences and noise (or, natural variation in the data). 
...
ii) As described above, variables that have small ranges are less powerful in classification. Classification ability is affected because these frames vary in the range of environmental variables -- for example, the dry desert plus Little's framework has the smallest variation in elevation between presence/absence sites. 
...
Your intended use statement can help you make a judgment call in selecting a frame. You should consider the extent and density of available data sources, as well as the spatial extent to which you want to apply the model. It's important to document your analysis decisions, and make defensible choices that build on previous work in your field. 
...
The summary values seem very similar over these four spatial extents, and the mean plus/minus standard deviations overlap for all three variables. 
---
